...
# we skip a one-time tip — strictly better than confusing a non-Code user. There
# is no documented env var that cleanly separates Code from the Agent SDK, so the
# allowlist keys on the observed Claude Code markers and is intentionally narrow;
# widen it only against a verified entrypoint value (see is_claude_code_surface).
#
...
is_claude_code_surface() {
    # CLAUDECODE=1 is set in every Claude-Code-spawned subprocess (incl. hooks).
    # Absent → not Code at all → silent.
    [[ "${CLAUDECODE:-}" == "1" ]] || return 1
    # Dispatch (the Claude Desktop autonomous agent) sets CLAUDE_CODE_BRIEF; it is
    # never an interactive Code surface, so exclude it explicitly even if a future
    # entrypoint value would otherwise match below.
    [[ -n "${CLAUDE_CODE_BRIEF:-}" ]] && return 1
    # Allowlist the entrypoints confirmed to be interactive Claude Code:
    #   cli       — the terminal CLI (including an IDE's integrated terminal)
    #   remote_*  — Claude Code on the web / mobile (remote execution, e.g.
    #               remote_mobile). These are real Code surfaces a human reads.

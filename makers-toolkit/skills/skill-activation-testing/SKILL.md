...
- Make every turn **self-contained — and remember a turn is text *plus environment*.** If it needs input the skill would first ask for ("deconstruct this essay" with no essay), the model asks for the input instead of firing and you've measured nothing. Deictic references ("this design," "this file") bind to whatever the working directory holds — in one run, to the test harness itself. Run each rep from an empty cwd, or write turns that carry their own referent.

...
**As of 2026-05-11.** Plugin is fully active. This note exists to document its overlap with `memory-substrate` and a potential refactor.
...
The `memory-substrate@external-cognition-dev` plugin (in CCP at `projects/ex-cog-dev/memory-substrate/`) ships a **byte-identical** copy of the 5 core source files for `relational-memory` (`backend.py`, `models.py`, `server.py`, `claude_agent.py`, `__init__.py`).
...
## Until then
No action needed on this plugin. It works. Just be aware that touching the relational-memory or edge-graph Python source here is the **source of truth** at runtime — memory-substrate's copies are dormant duplicates.

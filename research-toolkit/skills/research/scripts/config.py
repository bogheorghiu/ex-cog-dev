...
APP = "research-toolkit-investigation"
ENV_OVERRIDE = "RESEARCH_INVESTIGATION_CONFIG_DIR"
...
def describe(store):
    lines = [
        "research-toolkit investigation config store — interface (for the model; the user never runs this).",
        "",
        "Addressing: elements are category/name (lowercase, [a-z0-9-]); stored one per file.",
        "Known categories: profile (languages, deliverable, budget, saturation-threshold, engagement).",
        "",
        "Commands:",
    ]
